...
{
    public class Underkonsult :BaseTabel
    {
        #region Fields
        #endregion
        #region Methods
        #region private
        private StatistikTabel GetErastingValues(StatistikTabel tabel, string startDatum, string slutDatum)
        {
            tabel.AddValue(Constants.ColumnNames.UltaggKonstad,String.Empty);
            tabel.AddValue(Constants.ColumnNames.Milersättning, String.Empty);
            tabel.AddValue(Constants.ColumnNames.Traktamente, String.Empty);
            tabel.AddValue(Constants.ColumnNames.SummaTotalt, String.Empty);
            return tabel;
        }
        private StatistikTabel AddCalculateColumns(StatistikTabel tabel, DataTable table, List<string> columnsNamesToAdd)
        {
            ...
            tabel.AddValue(Constants.ColumnNames.ManmanaderTotalt, timmar / 168);
            tabel.AddValue(Constants.ColumnNames.DebiteringBemanning, debitering);
            tabel.AddValue(Constants.ColumnNames.Lönekonstnader, String.Empty);
            tabel.AddValue(Constants.ColumnNames.UKkonstnader, löne);
            ...
            tabel.AddValue(Constants.ColumnNames.TimmarOdebiterade, String.Empty);
        ...
        #endregion
        #region public
        public StatistikTabel GetStatistikTabel(string startDatum, string slutDatum, DataTable dataTabel, List<string> columnsNamesToAdd)
        {
            StatistikTabel result = new StatistikTabel(name);
            result = AddCalculateColumns(result, dataTabel, columnsNamesToAdd);
            result = GetErastingValues(result, startDatum, slutDatum);
            return result;
        }
        #endregion
        #endregion
        ...
        public Underkonsult(String _name, List<String> columnNames)
        {
        ...
        public Underkonsult(String _name, int type, List<String> queryColumns, List<String> columnNames,
            string startDatum, string slutDatum, List<string> columnsNamesToAdd, bool showColumns)
        {
            ...
            statistikTabel = GetStatistikTabel(startDatum, slutDatum, dataTabel, columnsNamesToAdd);
